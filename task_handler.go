...
func CreatTask(w http.ResponseWriter, r *http.Request) {
	...
	if task.Repeat != "" {
		...
		nextDate, err := CalculateNextDate(parsedDate, task.Date, task.Repeat)
		if err != nil {
...
func GetTask(w http.ResponseWriter, r *http.Request) {
	...
	err := Database.QueryRow("SELECT id, date, title, comment, repeat FROM scheduler WHERE id=?", id).Scan(&task.ID, &task.Date, &task.Title, &task.Comment, &task.Repeat)
...
func UpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	if task.Repeat != "" {
		nextDate, err := CalculateNextDate(time.Now(), task.Date, task.Repeat)
		if err != nil {
			http.Error(w, fmt.Sprintf(`{"error":"%s"}`, err.Error()), http.StatusBadRequest)
			return
		}
	...
	_, err = Database.Exec(query, task.Date, task.Title, task.Comment, task.Repeat, task.ID)
	if err != nil {
...
func DoneTask(w http.ResponseWriter, r *http.Request) {
	...
	var task Task
	err := Database.QueryRow("SELECT date, repeat FROM scheduler WHERE id = ?", id).Scan(&task.Date, &task.Repeat)
	if err != nil {
	...
	if task.Repeat == "" {
		if _, err = Database.Exec("DELETE FROM scheduler WHERE id = ?", id); err != nil {
	...
	if _, err := Database.Exec("UPDATE scheduler SET date = ? WHERE id = ?", nextDate, id); err != nil {
